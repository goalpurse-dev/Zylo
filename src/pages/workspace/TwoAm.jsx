...
export default function TwoAm() {
  ...
  useEffect(() => {
    if (loading) return;
    if (!user) setPaywallOpen(true);
  }, [user, loading]);

  const requestAuth = () => {
    if (loading) return true;
    if (!user) { setPaywallOpen(true); return true; }
    return false;
  };
  ...
  return (
    <>
      ...
      <FaceAsmrPaywall open={paywallOpen} onClose={() => setPaywallOpen(false)} isGuest={!user} dismissable={!!user} toolName="2AM In..." />
    </>
  );
}
