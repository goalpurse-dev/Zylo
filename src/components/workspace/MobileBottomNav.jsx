...
import { Home, Folder, LayoutGrid, Send } from "lucide-react";
import { createPortal } from "react-dom";
import MobileCreateMenu, { PUBLISH_TOOLS } from "./CreateMenu";
...
function PublishPopup({ open, onClose }) {
  const navigate = useNavigate();
  if (!open) return null;
  ...
  return createPortal(
    <div className="fixed inset-0 z-[190]" onClick={onClose}>
      ...
      <div
        className="absolute"
        style={{ bottom: "calc(72px + env(safe-area-inset-bottom) + 12px)", right: "calc(4%)" }}
      >
        <div className="flex gap-5" onClick={(e) => e.stopPropagation()}>
          {PUBLISH_TOOLS.map((item, i) => (
            <div
              key={item.id}
              className="flex flex-col items-center gap-2 transition-all"
              style={{
            ...
            >
              <button
                onClick={() => go(item.path)}
                className="flex h-[56px] w-[56px] items-center justify-center rounded-full border border-white/15 bg-[#1a1c24] text-white transition-transform active:scale-90"
              >
                {item.icon}
              </button>
              <span className="text-[11px] font-semibold text-white/80">{item.label}</span>
    ...
    <>
      ...
      >
        <div className={`flex items-center justify-between px-2 h-[${NAV_HEIGHT}px]`} style={{ height: NAV_HEIGHT }}>
          ...
          <button
            className="ftg-nav-item flex flex-1 justify-center"
            onClick={() => { setCreateOpen(false); setPublishOpen(false); setWorkspaceOpen((v) => !v); }}
          >
            <div className={`flex flex-col items-center justify-center w-[60px] h-[50px] rounded-xl transition-all duration-200 active:scale-95 ${
              workspaceOpen ||
          ...
          <button
            className="ftg-nav-item flex flex-1 justify-center"
            onClick={() => { setWorkspaceOpen(false); setPublishOpen(false); setCreateOpen((v) => !v); }}
          ...
          <NavItem
            ...
            active={publishOpen || location.pathname.startsWith("/workspace/publish") || location.pathname.startsWith("/workspace/stats") || location.pathname.startsWith("/workspace/connections")}
            onClick={() => { setCreateOpen(false); setWorkspaceOpen(false); setPublishOpen((v) => !v); }}
          />
