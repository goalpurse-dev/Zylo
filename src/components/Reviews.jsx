...
function ReviewCard({ data, variant = "center" }) {
  ...
  return (
    <div
      className={`rounded-[28px] bg-white/5 border border-white/10 shadow-sm px-8 sm:px-10 py-10 ${
    ...
    >
      ...
      <p className="mt-6 text-2xl sm:text-3xl font-extrabold leading-snug text-white text-center">
        “{data.quote}”
      </p>
      <div className="mt-5 text-sm font-semibold text-zinc-200 text-center">
        {data.name}
      </div>
      <div className="mt-7 text-[11px] uppercase tracking-wide text-zinc-400 text-center">
        Capterra
      </div>
      <div className="mt-2 flex justify-center gap-1 text-[#007BFF]">
        {Array.from({ length: 5 }).map((_, i) => (
          <Star key={i} className={`w-4 h-4 ${i < data.rating ? "" : "opacity-30"}`} />
        ))}
      </div>
    </div>
  );
      ...
      <div className="max-w-6xl mx-auto px-6 sm:px-8 py-14">
        ...
        <div className="relative mt-10 md:mt-12">
          ...
          <button
            ...
            className="absolute -left-8 md:-left-12 top-1/2 -translate-y-1/2 z-10 inline-flex items-center justify-center w-12 h-12 rounded-full bg-white/10 text-white border border-white/20 hover:bg-white/15 active:scale-95 transition"
          >
            ←
          </button>
          ...
          <button
            ...
            className="absolute -right-8 md:-right-12 top-1/2 -translate-y-1/2 z-10 inline-flex items-center justify-center w-12 h-12 rounded-full bg-white/10 text-white border border-white/20 hover:bg-white/15 active:scale-95 transition"
          >
            →
          </button>
